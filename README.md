...
The controlled-live chronology is preserved: the first pre-hardening run, the distinct later post-hardening run, and the final post-remediation controlled live validation each passed the implementation current at the time. After patch 3 changed the production operator, deadline, and telemetry boundaries, the final post-patch-3 controlled live validation **PASS**ed as the final authoritative release-gate run for one approved public Solana mainnet-beta wallet using `lookback_7d_v1`. It examined two pages, terminated with `historical_bound_reached`, observed 76 canonical signatures, excluded zero post-anchor signatures, reconciled five in-window and five Enhanced transactions, and retained the 1 supported, 0 unsupported, 1 ambiguous, 3 unrelated, and 0 failed partition. Patch 3 changed no aggregate classification relative to the prior run; the newer finalized anchor naturally changed the content-addressed digests. Zero candidates was a valid result, not a validation failure; live candidate resolution and Slice 7 were not exercised. The live release gate is complete, no further live rerun is required before tagging v1.14.0, and v1.14.0 is not yet tagged. Wallet-history completeness remains provider-attested, not trustless. See `engine/docs/v1.14-release-notes.md`, `engine/docs/v1.14-wallet-wide-acquisition.md`, `engine/docs/v1.14-operations.md`, and `engine/docs/v1.14-limitations.md`.
...
### v1.14 patch 3 release status and capability boundary

The authoritative `wallet_wide_acquisition_result_v1` exports now apply complete disposition accounting, canonical order/dense indexes, exact source/disposition/event/finding equality, quote semantics, and native Solana identity validation. Solana signatures decode from Base58 to exactly 64 bytes; wallets, mints, accounts, program IDs, and blockhashes decode to exactly 32 bytes. A wallet-history provider is usable only when its hidden acquisition-wide deadline starter successfully binds detached immutable request budgets before the first operation. Production control timing uses one monotonic `performance.now()` origin per acquisition.

The provider-neutral core has no global fetch, direct `process.env` access, credential-file reads, package store, publication, signing, minting, or deployment capability. The separately sanctioned controlled-live executable reads `HELIUS_API_KEY` by presence/value, uses global fetch through its HTTP client, and writes one sanitized mode-0600 temporary report; it never persists the key or raw provider bodies. `retry_count` counts retry attempts actually begun after the initial attempt. `timeout_count` counts HTTP attempts actually terminated by their effective transport timeout, once per attempt.

Deterministic regression performs no live operator run. It does execute offline controlled-live operator tests with injected ports, which may create isolated temporary sanitized report fixtures; no production, package-store, or publication write occurs. Deterministic remediation is complete only after the tests pass. The separately authorized final post-patch-3 controlled live validation passed, so the live release gate is complete and no further live rerun is required before tagging v1.14.0. The earlier live-validation chronology remains accurate, this post-patch-3 run is the final authoritative release-gate run, and v1.14.0 is not yet tagged.
