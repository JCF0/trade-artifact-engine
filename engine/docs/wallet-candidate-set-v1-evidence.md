...
The v1.14 adapter proves both latest-state acquisition from a null initial cursor and complete backward acquisition through the permitted lower bound. It fails closed on stale-head evidence, caps, truncation, timeout, provider uncertainty, malformed pagination, or exact enrichment-set mismatch. Every exported acquisition-result boundary now invokes complete disposition accounting, canonical ordering/dense indexing, exact reference/set equality, quote/finding semantics, and native Solana identity widths. The first pre-hardening controlled live validation, the distinct later post-hardening validation, and the final post-remediation controlled live validation all passed their respective implementations. After patch 3, the final post-patch-3 controlled live validation **PASS**ed as the final authoritative release-gate run on Solana mainnet-beta with `lookback_7d_v1`: two pages, `historical_bound_reached`, 76 canonical signatures, zero post-anchor signatures excluded, five in-window and five Enhanced transactions, and dispositions of 1 supported, 0 unsupported, 1 ambiguous, 3 unrelated, and 0 failed. Patch 3 changed no aggregate classification relative to the prior run; the newer finalized anchor naturally changed the content-addressed digests. Zero candidates was a valid result, not a validation failure; live candidate resolution and Slice 7 were not exercised, and no receipt or package digest was issued. The live release gate is complete, no further live rerun is required before tagging v1.14.0, and v1.14.0 is not yet tagged. Wallet-history completeness remains provider-attested, not trustless. The tracked tree intentionally contains exactly five retained Helius fixture bodies used for deterministic replay; controlled-live raw responses are not retained, and no exact retained finalized RPC transcript exists.
