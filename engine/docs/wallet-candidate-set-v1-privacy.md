...
The current schema enforces exact object shapes and stronger lexical restrictions for identifier-code fields, but generic identity strings such as wallet, transaction hash, blockhash and mint fields are currently only required to be nonempty. Until those fields receive equivalent lexical hardening, a trusted acquisition boundary must enforce the prohibition before construction; content addressing alone does not sanitize a sensitive-looking string.
