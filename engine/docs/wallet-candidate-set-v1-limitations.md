...
- Every validated acquisition result independently requires canonical normalized-event order and exact dense wallet-wide `raw_index` values; downstream evidence construction still canonicalizes the collections it owns.
...
- Plain-data validation bounds depth and total node count. Authoritative v1.14 boundaries additionally enforce native Base58 decoded widths for wallets, transaction signatures, blockhashes, mints, accounts, and program IDs; lexical privacy prohibitions remain fail-closed.
- The v1.14 adapter proves latest-state acquisition from a null initial cursor, a finalized coherent upper boundary, and lower-bound sentinel or provider exhaustion. Caps, truncation, stale-head evidence, timeout, protocol mismatch, or provider uncertainty produce no acquisition result. The first pre-hardening controlled live validation, the distinct later post-hardening validation, and the final post-remediation controlled live validation all passed their respective implementations. After patch 3, the final post-patch-3 controlled live validation **PASS**ed as the final authoritative release-gate run and retained the 1 supported, 0 unsupported, 1 ambiguous, 3 unrelated, and 0 failed partition. Patch 3 changed no aggregate classification relative to the prior run; the newer finalized anchor naturally changed the content-addressed digests. Zero candidates was valid and not a validation failure; live candidate resolution and Slice 7 were not exercised, and no receipt or package digest was issued. The live release gate is complete, no further live rerun is required before tagging v1.14.0, and v1.14.0 is not yet tagged. Wallet-history completeness remains provider-attested, not trustless. The tracked tree intentionally contains exactly five retained Helius fixture bodies; controlled-live raw responses are not retained, and no exact retained finalized RPC transcript exists.
...
Native identity validation now rejects non-Base58 and wrong decoded widths at acquisition, evidence, and candidate boundaries. The provider-neutral core and the controlled-live executable have intentionally different capability sets; offline operator tests may create isolated sanitized temporary reports, while deterministic regression supplies no live provider request.
