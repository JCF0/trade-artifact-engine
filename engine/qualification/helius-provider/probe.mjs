...
import { createSession, loadExchange, put } from './bounded.mjs';
...
export async function probe(session, root) {
  const findings = { classification: 'PROVIDER_SAMPLE_NOT_READINESS', passed: [], not_tested: [
    'SETUP_TRANSACTION_BODIES_NO_FROZEN_SIGNATURE_REFERENCES',
    'SUPPORTED_BINARY_LAYOUT_AND_DYNAMIC_TICK_ARRAY_VALIDATION',
    'EXACT_MESSAGE_FEE_AND_UNSIGNED_SIMULATION_NO_ADMITTED_BUILDER_INPUT',
  ...
  try {
    ...
    const first = atFloor(await session.call('getAccountInfo', [PUBLIC.pool, config(slot)]), slot); account(first.value);
    const routeAddresses = [PUBLIC.pool, PUBLIC.jup_vault, PUBLIC.usdc_vault, PUBLIC.oracle, PUBLIC.jup_mint, PUBLIC.usdc_mint];
    const route = atFloor(await session.call('getMultipleAccounts', [routeAddresses, config(first.context.slot)]), first.context.slot);
    check(Array.isArray(route.value) && route.value.length === routeAddresses.length); route.value.forEach(account);
    findings.contexts.push({ method: 'getAccountInfo', requested: slot, observed: first.context.slot },
      { method: 'getMultipleAccounts', requested: first.context.slot, observed: route.context.slot });
    const latest = atFloor(await session.call('getLatestBlockhash', [{ commitment: 'finalized', minContextSlot: route.context.slot }]), route.context.slot);
    check(typeof latest.value?.blockhash === 'string' && integer(latest.value.lastValidBlockHeight));
    const height = await session.call('getBlockHeight', [{ commitment: 'finalized', minContextSlot: latest.context.slot }]);
    check(integer(height) && height < latest.value.lastValidBlockHeight);
    findings.blockhash = { ...latest.value, context: latest.context, observed_height: height };
    findings.passed.push('BLOCKHASH_AND_HEIGHT_SAMPLE');
...
// No automatic entry dispatch. The unprivileged worker must be invoked explicitly
// by a separately established custodian launcher, not a production live entry point.
export async function runQualifiedReads(root) { return probe(createSession(loadExchange(), root), root); }
