"""Complete entry integration; synthetic loopback TLS only, no actual credential read."""
import base64
import copy
import hashlib
import http.server
...
def run_case(root, case, cert, key, expect_timeout=False):
    ...
    def response(body):
        ...
        if method == 'getMultipleAccounts':
            ...
            return at(copy.deepcopy(ROUTE) + [account(route['whirlpool_program']), account(wallet['token_program'], bytes(82)), account(wallet['token_program'], bytes(82))])
        if method == 'getTokenAccountsByOwner':
            assert params[0] == wallet['wallet'] and params[2] == dict(commitment='finalized', encoding='base64', minContextSlot=SLOT)
            return at([dict(pubkey=addresses[i], account=opening[i]) for i in (1, 2)] if params[1]['programId'] == wallet['token_program'] else [])
    ...
    class Handler(http.server.BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            try:
                ...
                raw = ('  ' + json.dumps(dict(jsonrpc='2.0', id=body['id'], result=result), separators=(', ', ': ')) + '\n').encode()
                received.append(dict(method=body['method'], request=request, response=raw))
    ...
    try: result = launcher.run('worker-local', destination, server.server_port, cert.read_text(), case)
    ...
    try:
        ...
        if expect_timeout:
            ...
            assert result['exit'] == (0 if case == 'complete' else 1) and not result['timeout']
        if expect_timeout:
            pass
            ...
            assert ledger['consumed'] == len(received) == (16 if case == 'setup-contradiction' else 23)
            assert ledger['counts'] == counts and ledger['admitted_bytes'] == sum(len(r['response']) for r in received)
            ...
            if case == 'complete':
                ...
                if case == 'setup-contradiction': assert 'simulateTransaction' not in counts
                else: assert json.loads((evidence / 'simulation-disposition.json').read_bytes())['outcome'] == 'OBSERVED_UNSIGNED_PROGRAM_REFUSAL'
...
def run(destination, selection):
    ...
    cases = ['complete', 'input-extra', 'setup-contradiction', 'simulation-refusal'] if selection == 'all' else [selection]
    assert all(c in ('complete', 'input-extra', 'setup-contradiction', 'simulation-refusal') for c in cases)
    launcher.base.put(root, 'dispatch.json', dict(classification='SYNTHETIC_ONLY_NOT_PROVIDER', cases=cases,
        source_before={p.name: launcher.base.sha(p) for p in HERE.iterdir() if p.is_file()}, external_session_consumed=False))
    with tempfile.TemporaryDirectory(prefix='artifact-worker-synthetic-tls-') as directory:
        cert, key = Path(directory) / 'cert.pem', Path(directory) / 'tls-fixture-key.pem'
        subprocess.run(['/usr/bin/openssl', 'req', '-x509', '-newkey', 'rsa:2048', '-nodes', '-days', '1', '-keyout', str(key), '-out', str(cert),
            '-subj', '/CN=mainnet.helius-rpc.com', '-addext', 'subjectAltName=DNS:mainnet.helius-rpc.com'], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        results = [run_case(root, case, cert, key) for case in cases]
    before = json.loads((root / 'dispatch.json').read_bytes())['source_before']
    assert before == {p.name: launcher.base.sha(p) for p in HERE.iterdir() if p.is_file()}
    launcher.base.put(root, 'summary.json', dict(classification='SYNTHETIC_ONLY_NOT_PROVIDER', cases=cases, passed=results,
        source_after=before, tls_temporary_root_removed=not Path(directory).exists(), real_credential_reads=0, external_requests=0))
