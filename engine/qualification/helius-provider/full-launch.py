...
base: Any = importlib.util.module_from_spec(spec); spec.loader.exec_module(base)
base.RUNTIME = Path('/root/artifact-private-helius-provider-qualification/first-exchange-diagnostic-v1/runtime-successor-v1/runtime-current.json')
base.R = 'a07f1bd10f71521b5ce95cd9273367ade083741d1bd96b3b3071d1a9a38c6ebd'
base.HERE = HERE
...
E = Path('/root/artifact-private-helius-provider-qualification/full-dedicated-v1')
...
def run_managed(mode, destination, fixture_port, fixture_ca, fixture_case,
    ...
    result: dict[str, Any] = dict(mode=mode, released=False, real_provider_requests=0 if mode != 'live' else None)
    try:
        ...
        if mode == 'live':
            ...
            base.put(E, 'single-session-reservation.json', dict(previous_allowances='THREE_CONSUMED_UNCHANGED', status='CONSUMED_POSSIBLY_DISPATCHED', calls=29, simulations=1, retries=0))
            ...
            credential_fd = deliver_dedicated(workspace) if mode == 'worker-local' else custody.deliver(workspace, bytearray(b'SYNTHETIC_QUALIFICATION_KEY_0001'))
        ...
        command = ['/usr/bin/unshare', '--mount', '--pid', '--fork', '--kill-child=KILL', '/usr/bin/python3', '-B',
            str(HERE / 'full-launch.py'), '--child', str(jail), mode, str(fixture_port), os.readlink('/proc/self/ns/pid'), os.readlink('/proc/self/ns/net')]
