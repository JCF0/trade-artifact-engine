// Qualification-only versioned successor. The zero-retry predecessor is unchanged.
import { createHash } from 'node:crypto';
import { performance } from 'node:perf_hooks';
import { setTimeout as sleep } from 'node:timers/promises';
import { canonicalJson } from '/accepted/engine/src/verification-scope-v1-3/contract.mjs';
import { MAXIMA, LIMITS, put, loadExchange } from './bounded-successor.mjs';
export { MAXIMA, LIMITS, put, loadExchange };
export const VERSION = 'ARTIFACT_QUALIFICATION_CONTEXT_LAG_SESSION_V1';
...
export function createSession(exchange, root, workDeadlineMonotonicMs, limits = LIMITS) {
  ...
  return Object.freeze({
    ...
    snapshot() { return JSON.parse(JSON.stringify({ version: VERSION, policy: RETRY_POLICY, limits,
