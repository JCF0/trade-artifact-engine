...
spec = importlib.util.spec_from_file_location('custody', HERE / 'artifact-custody.py')
...
E = Path('/root/artifact-private-helius-provider-qualification/dedicated-credential-handoff-v2/diagnostic-session')
ENDPOINT_HOST = 'mainnet.helius-rpc.com'

def require_operator_entry(ready):
    """An explicit operator assertion, never inferred from credential existence.

    READY still needs separate new external authorization. Lost/failed entry
    completion cannot be promoted by parsing the published file or by retrying.
    """
    entry = ready.get('credential_entry')
    custody.base.require(type(entry) is dict and set(entry) == {'source', 'operator_confirmed', 'exit', 'marker'},
                         'CREDENTIAL_ENTRY_UNCONFIRMED')
    custody.base.require(entry['source'] == custody.SOURCE and entry['operator_confirmed'] is True
                         and type(entry['exit']) is int and entry['exit'] == 0
                         and entry['marker'] == 'ARTIFACT_CREDENTIAL_ENTRY_PUBLISHED',
                         'CREDENTIAL_ENTRY_UNCONFIRMED')

def deliver_dedicated(workspace):
    return custody.deliver(workspace, custody.read_authorized_key())

_sequence = 0
def observation(role, stage, error=None, value=0):
    global _sequence
    assert role in ('broker', 'relay') and stage in ('ACCEPTED', 'REQUESTED', 'DNS_COMPLETE',
        'TCP_CONNECTED', 'FD_SENT', 'FD_RECEIVED', 'EOF', 'STOP')
    categories = {socket.EAI_AGAIN: 'DNS_TEMPORARY', socket.EAI_NONAME: 'DNS_NOT_FOUND'}
    code = 'NONE'
    if error is not None:
        import errno
        code = categories.get(error.errno, 'UNKNOWN') if isinstance(error, socket.gaierror) else {
            errno.ECONNREFUSED: 'TCP_REFUSED', errno.ECONNRESET: 'CONNECTION_RESET',
            errno.EAGAIN: 'WOULD_BLOCK', errno.EPIPE: 'BROKEN_PIPE', errno.ETIMEDOUT: 'TIMEOUT'
        }.get(getattr(error, 'errno', None), 'UNKNOWN')
        if isinstance(error, TimeoutError): code = 'TIMEOUT'
    assert isinstance(value, int) and 0 <= value <= 16777216 and _sequence < 40
    _sequence += 1
    base.put(Path('/evidence'), f'diagnostic-{role}-{_sequence:02d}.json',
        dict(version=1, stage=stage, code=code, value=value, monotonic_ns=time.monotonic_ns()))
...
def drop(jail, keep, role):
    ...
    assert not Path('/root/.artifact-qualification').exists()
            ...
            with socket.socket(fileno=descriptors[0]) as upstream:
                ...
                while True:
                    ...
                    for source in readable:
                        ...
                        if not data: observation('relay', 'EOF'); ended = True; break
...
def child(jail_name, mode, fixture_port, parent_pid_ns, parent_net_ns):
    ...
    if pid == 0:
        ...
        if relay == 0:
            ...
            except Exception as error: observation('relay', 'STOP', error)
            finally: os._exit(0)
        ...
        entry = '/harness/diagnostic-worker.mjs'
        assert time.monotonic() < work_end
    ...
    try:
        ...
        while connections < 1:
            ...
            assert choices
            observation('broker', 'DNS_COMPLETE', value=len(choices))
...
def run_managed(mode, destination, fixture_port, fixture_ca, fixture_case,
    ...
    try:
        ...
        if mode == 'live':
            ...
            assert ready['classification'] == 'ONE_GENESIS_DIAGNOSTIC_ONLY'
            assert ready['method'] == 'getGenesisHash' and ready['params'] == []
            assert ready['calls'] == 1 and ready['retries'] == 0
            assert ready['local_checks_passed'] is True
            require_operator_entry(ready)
            for path, expected in ready['files'].items(): assert base.sha(Path(path)) == expected
            base.put(E, 'diagnostic-reservation.json', dict(original_qualification_allowance='CONSUMED_UNCHANGED', status='CONSUMED_POSSIBLY_DISPATCHED', calls=1, method='getGenesisHash', retries=0))
            in_time()
            credential_fd = deliver_dedicated(workspace)
        else:
            # Diagnostic-local provides only a test filesystem root, not a
            # replacement credential reader/deliverer. Same delivery as live.
            credential_fd = deliver_dedicated(workspace) if mode == 'local' else custody.deliver(workspace, bytearray(b'SYNTHETIC_QUALIFICATION_KEY_0001'))
            if fixture_ca is not None:
                os.close(credential_fd)
                path = workspace / 'private-fd5'; path.chmod(0o600)
                capability = json.loads(path.read_bytes())
                capability['ca'] = fixture_ca
                path.write_text(json.dumps(capability, sort_keys=True, indent=2) + '\n'); path.chmod(0o400)
        ...
        command = ['/usr/bin/unshare', '--mount', '--pid', '--fork', '--kill-child=KILL', '/usr/bin/python3', '-B',
            str(HERE / 'diagnostic-launch.py'), '--child', str(jail), mode, str(fixture_port), os.readlink('/proc/self/ns/pid'), os.readlink('/proc/self/ns/net')]
