...
function isSingleSupportedShape(source, operations) {
  if (source.fee_payer !== source.wallet || operations.length !== 2) return false;
  if (!source.recognized_programs.some(program => program.program_role === 'spot_swap')) return false;
  if (operations.some(operation => operation.operation_kind !== 'swap')) return false;
  if (new Set(operations.map(operation => operation.economic_group)).size !== 1) return false;
  if (operations.some(operation => operation.economic_group === null)) return false;
  const directions = operations.map(operation => operation.direction).sort(compareCodeUnits);
  if (directions[0] !== 'credit' || directions[1] !== 'debit') return false;
  const quoteCount = operations.filter(operation => QUOTE_MINTS_V1.includes(operation.mint)).length;
  return quoteCount === 1 && operations.every(operation => operation.mint !== null);
}
...
function invokeNormalizer(normalizer, source, operations) {
  ...
  try {
    ...
    assertExactFieldsV1(result, NORMALIZER_RESULT_FIELDS, 'normalization_failed');
...
function reconcileEvent(source, operations, event) {
  ...
  const debit = operations.find(operation => operation.direction === 'debit');
  ...
  const exact = event.wallet === source.wallet
    ...
    && event.token_in_amount === debit.amount
...
export function classifyWalletSourceTransactionV1(input) {
  ...
  if (!isSingleSupportedShape(source, operations)) {
  ...
  const normalized = invokeNormalizer(normalizer, source, operations);
  ...
  const eventRecord = reconcileEvent(source, operations, normalized.event);
