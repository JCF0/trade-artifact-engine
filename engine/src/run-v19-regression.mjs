...
const SUITES = [
  ...
  { name: 'Slice 6A Receipt Board View Model', file: 'engine/src/receipt-board/view-model.test.mjs' },
  { name: 'Slice 6B Receipt Board HTML', file: 'engine/src/receipt-board/render-html.test.mjs' },
  { name: 'Slice 6C Receipt Board API', file: 'engine/src/api/receipt-board-api.test.mjs' },
  { name: 'Slice 7A Coverage Statement View Model', file: 'engine/src/coverage-statement/view-model.test.mjs' },
  { name: 'Slice 8A Same-Mint Aggregation Helper', file: 'engine/src/pipeline/same-mint-input-aggregation.test.mjs' },
  { name: 'Slice 8B Same-Mint Aggregation Parity', file: 'engine/src/pipeline/same-mint-input-aggregation-parity.test.mjs' },
  { name: 'Slice 8C Same-Mint Real Shape', file: 'engine/src/pipeline/same-mint-input-real-shape.test.mjs' },
  { name: 'Slice 8D Classifier', file: 'engine/src/pipeline/test-classifier.mjs' },
  { name: 'Slice 9A Receipt Archive Store', file: 'engine/src/inventory/archive-store.test.mjs' },
  { name: 'Slice 9B Current Run Archive Import', file: 'engine/src/inventory/archive-current-run.test.mjs' },
  { name: 'Slice 9C Archive-Backed Inventory', file: 'engine/src/inventory/archive-inventory.test.mjs' },
  { name: 'Slice 9D Archive Resolution API', file: 'engine/src/api/archive-resolution-api.test.mjs' },
];
...
function summarizeOutput(output) {
  ...
  if (totalMatch) return `${totalMatch[1]} passed, ${totalMatch[2]} failed`;

  const allPassMatch = normalized.match(/ALL (\d+) CHECKS PASSED/i);
  if (allPassMatch) return `${allPassMatch[1]}/${allPassMatch[1]} passed, 0 failed`;

  const namedPassMatch = normalized.match(/:\s*(\d+) passed,\s*(\d+) failed/i);
  if (namedPassMatch) return `${namedPassMatch[1]} passed, ${namedPassMatch[2]} failed`;
...
console.log('Trade Artifact v1.9 Regression Runner');
console.log('Deterministic local suites only. No live Helius, wallet pipeline runs, upload, mint, signing, browser automation, mutable-cache pipeline tests, remote deploy, or network calls.');
