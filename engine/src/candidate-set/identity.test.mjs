...
const WALLET = providerPublicKey('wallet'); const OTHER_WALLET = providerPublicKey('other-wallet');
const MINT = providerPublicKey('mint'); const QUOTE = providerPublicKey('quote');
const tx = buildSourceTransactionReferenceV1({ block_time: 1700000000, slot: 42, tx_hash: providerSignature('tx') });
...
const boundary = { boundary_version: 'solana_finalized_acquisition_boundary_v1', chain: 'solana', network: 'mainnet-beta', genesis_hash: GENESIS_HASH, commitment: 'finalized', anchor_slot: 100, anchor_block_time: 2, anchor_blockhash: providerPublicKey('blockhash'), history_complete_through_anchor: true, lower_bound_completion_proven: true, boundary_status: 'proven' };
...
const otherProjection = structuredClone(projection); otherProjection.wallet = OTHER_WALLET;
const otherCandidate = buildCandidateV1({ ledger_candidate_hash: d('d'), receipt_scoped_evidence_digest: d('e'), selection_key: { wallet: OTHER_WALLET, token_mint: MINT, receipt_type: 'closed_position', segment_index: 0 }, projection: otherProjection });
