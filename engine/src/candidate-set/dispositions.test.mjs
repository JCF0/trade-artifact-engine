...
const SUPPORTED_TX = providerSignature('supported-tx');
const UNSUPPORTED_TX = providerSignature('unsupported-tx');
const event = buildEventRecordV1({
  source_slot: 9,
  slice7_event: {
    wallet, timestamp: 90, tx_hash: SUPPORTED_TX, source: 'swap',
...
const supported = buildDispositionV1({
  tx_hash: SUPPORTED_TX, slot: 9, block_time: 90,
...
const unsupportedSource = { tx_hash: UNSUPPORTED_TX, slot: 8, block_time: 80 };
...
assert.deepEqual(canonical.map(item => item.tx_hash), [UNSUPPORTED_TX, SUPPORTED_TX]);
...
wrongMint.affected_token_mints = [USDC_MINT, TOKEN].sort();
assert.throws(() => validateDispositionAccountingV1({
...
const nullableSource = { tx_hash: providerSignature('null-time'), slot: 6, block_time: null };
