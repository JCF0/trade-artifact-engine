...
const FINDING_REASON_CODES = new Set(['ambiguous_swap_direction','unsupported_swap_shape','unsupported_transfer_activity']);
...
function publicKey(value, field) { if (!isSolanaPublicKeyV1(value)) fail('invalid_field', `${field} must be a 32-byte Base58 Solana identity`, { field }); }
function signature(value, field = 'tx_hash') { if (!isSolanaSignatureV1(value)) fail('invalid_field', `${field} must be a 64-byte Base58 Solana signature`, { field }); }
...
function compareCandidateSetFindings(left, right) {
...
export function validateSourceTransactionReferenceV1(value) {
  ...
  signature(value.tx_hash); safe(value.slot, 'slot'); safe(value.block_time, 'block_time', true); return true;
}
export function validateProfilesV1(value) {
  assertExactFields(value, ['wallet_acquisition_profile','wallet_normalization_profile','reconstruction_engine_version','accounting_method_version','mark_profile','mark_max_age_seconds'], 'profiles');
  ...
  if (value.mark_profile === null) {
    if (value.mark_max_age_seconds !== null) fail('unsupported_profile', 'mark freshness policy requires a mark profile');
  } else if (value.mark_profile !== MARK_PROFILE_VERSION || value.mark_max_age_seconds !== MARK_PROFILE_MAX_AGE_SECONDS) fail('unsupported_profile', 'mark profile freshness policy is unsupported');
  return true;
}
export function validateBoundaryV1(value) {
  ...
  safe(value.anchor_slot, 'anchor_slot'); safe(value.anchor_block_time, 'anchor_block_time'); publicKey(value.anchor_blockhash, 'anchor_blockhash');
...
export function validateFindingV1(value, { verifyDigest = true } = {}) {
  ...
  oneOf(value.finding_type, ['unsupported_activity','ambiguous_activity'], 'finding_type');
  ...
  orderedUniqueStrings(value.affected_token_mints, 'affected_token_mints'); orderedUniqueStrings(value.affected_quote_mints, 'affected_quote_mints'); value.affected_token_mints.forEach(mint => publicKey(mint, 'affected_token_mints')); value.affected_quote_mints.forEach(mint => publicKey(mint, 'affected_quote_mints'));
...
export function validateDispositionV1(value, { verifyDigest = true } = {}) {
  ...
  orderedUniqueStrings(value.affected_token_mints, 'affected_token_mints'); value.affected_token_mints.forEach(mint => publicKey(mint, 'affected_token_mints')); orderedUniqueStrings(value.normalized_event_digests, 'normalized_event_digests', { digests: true }); orderedUniqueStrings(value.finding_digests, 'finding_digests', { digests: true });
...
export function validateSlice7EventV1(value) {
  assertExactFields(value, SLICE7_EVENT_FIELDS, 'slice7_event'); publicKey(value.wallet, 'wallet'); signature(value.tx_hash); publicKey(value.token_in_mint, 'token_in_mint'); publicKey(value.token_out_mint, 'token_out_mint'); for (const field of ['source','extraction_method']) identifierCode(value[field], field);
...
function validateEconomics(value, type) {
  ...
  for (const field of fields.filter(field => !['economics_type','accounting_method','close_reason_code','dust_classification_code'].includes(field))) finite(value[field], field, { nullable: field === 'realized_pnl_to_date_pct' }); safe(value.entry_count, 'entry_count'); safe(value.exit_count, 'exit_count'); for (const field of ['close_reason_code','dust_classification_code']) if (Object.hasOwn(value, field) && value[field] !== null) nonempty(value[field], field);
}
function validateSnapshot(value) {
  ...
  assertExactFields(value.mark, ['status','mark_profile','mark_price_raw_quote','mark_observed_at','mark_source_slot','freshness_status','reason_code'], 'snapshot.mark'); oneOf(value.mark.status, ['available','unavailable'], 'mark.status'); if (value.mark.mark_profile !== MARK_PROFILE_VERSION) fail('unsupported_profile', 'snapshot mark profile is unsupported'); oneOf(value.mark.freshness_status, ['fresh','stale','unavailable','quote_mismatch','after_boundary'], 'freshness_status');
  if (value.mark.status === 'available') { finite(value.mark.mark_price_raw_quote, 'mark_price_raw_quote', { positive: true }); safe(value.mark.mark_observed_at, 'mark_observed_at'); safe(value.mark.mark_source_slot, 'mark_source_slot'); if (value.mark.reason_code !== null || value.mark.freshness_status !== 'fresh') fail('invalid_field', 'available mark reason/freshness must be fresh and null'); if (value.mark.mark_source_slot > value.source_boundary.source_slot || value.mark.mark_observed_at > value.snapshot_at) fail('mark_observation_invalid', 'available mark lies after the snapshot boundary'); if (value.snapshot_at - value.mark.mark_observed_at > MARK_PROFILE_MAX_AGE_SECONDS) fail('mark_observation_invalid', 'available mark exceeds the frozen freshness policy'); } else { if (value.mark.mark_price_raw_quote !== null || value.mark.mark_observed_at !== null || value.mark.mark_source_slot !== null) fail('invalid_field', 'unavailable mark values must be null'); nonempty(value.mark.reason_code, 'mark.reason_code'); const expectedFreshness = ({ mark_stale:'stale', mark_quote_mismatch:'quote_mismatch', mark_after_snapshot_boundary:'after_boundary' })[value.mark.reason_code] ?? (['mark_source_unavailable','snapshot_boundary_unavailable'].includes(value.mark.reason_code) ? 'unavailable' : null); if (expectedFreshness === null || value.mark.freshness_status !== expectedFreshness) fail('invalid_field', 'unavailable mark reason/freshness is inconsistent'); }
...
export function validateCandidateV1(value, { verifyDigest = true } = {}) {
  ...
  assertExactFields(value.selection_key, ['wallet','token_mint','receipt_type','segment_index'], 'selection_key'); publicKey(value.selection_key.wallet, 'wallet'); publicKey(value.selection_key.token_mint, 'token_mint'); oneOf(value.selection_key.receipt_type, ['closed_position','realized_partial','open_snapshot'], 'receipt_type'); safe(value.selection_key.segment_index, 'segment_index');
  const pfields = ['candidate_type','position_status','ledger_evidence_status','boundary_status','valuation_status','selection_status','package_eligibility','economics_status','chain','network','wallet','token_mint','quote_mint','quote_symbol_code','segment_index','first_event_at','last_event_at','event_counts','ledger_eligibility','economics','snapshot','flags','limitations','reason_codes','disclosure_codes']; assertExactFields(value.projection, pfields, 'candidate.projection'); const p = value.projection; oneOf(p.candidate_type, ['closed_position','realized_partial','open_snapshot'], 'candidate_type'); oneOf(p.position_status, ['closed','open'], 'position_status'); oneOf(p.ledger_evidence_status, ['clean','limited_partial_history'], 'ledger_evidence_status'); oneOf(p.boundary_status, ['not_applicable','proven','unavailable'], 'boundary_status'); oneOf(p.valuation_status, ['raw_quote','mark_available','mark_unavailable','mark_stale','mark_quote_mismatch','mark_after_boundary','unavailable'], 'valuation_status'); oneOf(p.selection_status, ['selectable','visible_only'], 'selection_status'); oneOf(p.package_eligibility, ['eligible_closed_position_v1','not_publication_eligible_v1'], 'package_eligibility'); oneOf(p.economics_status, ['available','unavailable_partial_history'], 'economics_status'); if (p.chain !== 'solana' || p.network !== 'mainnet-beta') fail('invalid_chain_identity', 'candidate chain identity is invalid'); publicKey(p.wallet, 'wallet'); publicKey(p.token_mint, 'token_mint'); publicKey(p.quote_mint, 'quote_mint'); nonempty(p.quote_symbol_code, 'quote_symbol_code'); safe(p.segment_index, 'segment_index'); safe(p.first_event_at, 'first_event_at'); safe(p.last_event_at, 'last_event_at'); if (p.first_event_at > p.last_event_at) fail('invalid_field', 'candidate event range is reversed');
  assertExactFields(p.event_counts, ['buys','sells','supported_events','associated_findings'], 'event_counts'); for (const f of Object.keys(p.event_counts)) safe(p.event_counts[f], f); assertExactFields(p.ledger_eligibility, ['eligible_for_closed_position','eligible_for_verified_receipt'], 'ledger_eligibility'); if (typeof p.ledger_eligibility.eligible_for_closed_position !== 'boolean' || typeof p.ledger_eligibility.eligible_for_verified_receipt !== 'boolean') fail('invalid_field', 'ledger eligibility must be boolean'); for (const f of ['flags','limitations','reason_codes','disclosure_codes']) orderedUniqueStrings(p[f], f);
  const limited = p.ledger_evidence_status === 'limited_partial_history';
  if (limited) {
    if (p.economics_status !== 'unavailable_partial_history' || p.economics !== null || p.snapshot !== null || p.valuation_status !== 'unavailable' || p.selection_status !== 'visible_only' || p.package_eligibility !== 'not_publication_eligible_v1') fail('invalid_candidate_status', 'limited partial-history candidate availability axes are invalid');
    if (!p.reason_codes.includes('partial_history_boundary') || !p.reason_codes.includes('unobserved_pre_window_inventory') || !p.disclosure_codes.includes('history_begins_mid_position') || !p.disclosure_codes.includes('activity_not_reconstructable')) fail('invalid_candidate_status', 'limited partial-history candidate disclosures are incomplete');
  } else {
    if (p.economics_status !== 'available') fail('invalid_candidate_status', 'clean candidate economics must be available');
    validateEconomics(p.economics, p.candidate_type); if ((p.candidate_type === 'closed_position') !== (p.snapshot === null)) fail('invalid_field', 'snapshot type invariant failed'); if (p.snapshot !== null) validateSnapshot(p.snapshot);
  }
  const isClosed = p.candidate_type === 'closed_position';
  if ((isClosed && p.position_status !== 'closed') || (!isClosed && p.position_status !== 'open')) fail('invalid_candidate_status', 'candidate type and position status are inconsistent');
  if ((isClosed && p.boundary_status !== 'not_applicable') || (!isClosed && p.boundary_status !== 'proven')) fail('invalid_candidate_status', 'candidate boundary status is inconsistent');
  if (p.event_counts.buys + p.event_counts.sells > p.event_counts.supported_events) fail('candidate_event_count_mismatch', 'candidate event counts do not reconcile');
  if (!limited && (p.economics.entry_count !== p.event_counts.buys || p.economics.exit_count !== p.event_counts.sells)) fail('candidate_event_count_mismatch', 'candidate economics counts do not reconcile');
  if (isClosed && !limited) {
    if (p.valuation_status !== 'raw_quote' || p.economics.remaining_qty !== 0 || p.economics.remaining_cost_basis_quote !== 0 || p.economics.hold_time_seconds !== p.last_event_at - p.first_event_at) fail('invalid_candidate_status', 'closed candidate economics, duration, or valuation status is inconsistent');
  } else if (!limited) {
    const markValuation = p.snapshot.mark.status === 'available' ? 'mark_available' : ({ mark_stale:'mark_stale', mark_quote_mismatch:'mark_quote_mismatch', mark_after_snapshot_boundary:'mark_after_boundary' })[p.snapshot.mark.reason_code] ?? 'mark_unavailable';
    if (p.valuation_status !== markValuation || p.economics.remaining_qty !== p.snapshot.remaining_qty || p.economics.remaining_cost_basis_quote !== p.snapshot.remaining_cost_basis_quote) fail('invalid_candidate_status', 'open candidate snapshot and projection are inconsistent');
    const realizedQuote = p.candidate_type === 'realized_partial' ? p.economics.realized_pnl_quote : p.economics.realized_pnl_to_date_quote;
    const realizedPct = p.candidate_type === 'realized_partial' ? p.economics.realized_pnl_pct : p.economics.realized_pnl_to_date_pct;
    if (realizedQuote !== p.snapshot.realized_pnl_to_date_quote || realizedPct !== p.snapshot.realized_pnl_to_date_pct || p.snapshot.disclosure_codes.some(code => !p.disclosure_codes.includes(code))) fail('invalid_candidate_status', 'open candidate economics or disclosures do not match its snapshot');
  }
...
function validateBlockedSummaryRelations(payload) {
  ...
  for (const finding of payload.activity_findings) {
    if (finding.impact_scope !== 'token_specific') fail('wallet_wide_impact_unresolved', 'candidate sets cannot contain wallet-wide findings');
  ...
  for (const summary of payload.blocked_summaries) {
    if (summaryTokens.has(summary.token_mint)) fail('blocked_summary_mismatch', 'blocked summaries must be unique by token');
    summaryTokens.add(summary.token_mint);
    const findings = findingsByToken.get(summary.token_mint);
    if (findings === undefined) fail('blocked_summary_mismatch', 'blocked summary token is not affected by a finding');
    const expectedStatus = findings.some(finding => finding.finding_type === 'ambiguous_activity') ? 'blocked_ambiguous_activity' : 'blocked_unsupported_activity';
    const expectedDigests = findings.map(finding => finding.finding_digest).sort(compareCodeUnits);
    const expectedReasons = [...new Set(findings.flatMap(finding => finding.reason_codes))].sort(compareCodeUnits);
    const expectedDisclosures = [...new Set(findings.flatMap(finding => finding.disclosure_codes))].sort(compareCodeUnits);
    if (summary.ledger_evidence_status !== expectedStatus || sha256CanonicalJson(summary.associated_finding_digests) !== sha256CanonicalJson(expectedDigests) || sha256CanonicalJson(summary.reason_codes) !== sha256CanonicalJson(expectedReasons) || sha256CanonicalJson(summary.disclosure_codes) !== sha256CanonicalJson(expectedDisclosures)) fail('blocked_summary_mismatch', 'blocked summary does not reconcile with activity findings');
  }
  if (summaryTokens.size !== findingsByToken.size) fail('blocked_summary_mismatch', 'every finding-affected token requires one blocked summary');
}
export function validateCandidateSetV1(value) {
  ...
  validateBlockedSummaryRelations(value.payload);
  for (const field of ['candidates','blocked_summaries']) {
    const digests = value.payload[field].map(x => x.candidate_digest ?? x.blocked_summary_digest);
    if (new Set(digests).size !== digests.length) fail('duplicate_value', `${field} contains duplicate digests`);
  ...
  const findingDigests = value.payload.activity_findings.map(x => x.finding_digest);
  if (new Set(findingDigests).size !== findingDigests.length) fail('duplicate_value', 'activity_findings contains duplicate digests');
  for (let index = 1; index < value.payload.activity_findings.length; index += 1) if (compareCandidateSetFindings(value.payload.activity_findings[index - 1], value.payload.activity_findings[index]) >= 0) fail('order_invalid', 'activity_findings is not in canonical finding order');
