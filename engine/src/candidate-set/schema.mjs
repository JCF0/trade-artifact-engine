...
export function validateCandidateV1(value, { verifyDigest = true } = {}) {
  ...
  if (p.economics.entry_count !== p.event_counts.buys || p.economics.exit_count !== p.event_counts.sells || p.event_counts.buys + p.event_counts.sells > p.event_counts.supported_events) fail('candidate_event_count_mismatch', 'candidate event counts do not reconcile');
  if (isClosed) {
