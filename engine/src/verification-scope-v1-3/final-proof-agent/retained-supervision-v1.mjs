...
export function loadRetainedSupervisionV1(loaded, control) {
  ...
  const identity = control.mandate.mandate_profile === OFFLINE_WALLET_PROFILE_V1 ? control.mandate.offline_identity : control.mandate.unresolved_live_readiness;
...
export function validateRetainedRpcPhaseV1(records, budget, phase) {
...
export async function validateRetainedSimulationV1({ loaded, control, leg, plan, capture }) {
  ...
  const { pairs, unfinished } = validateRetainedRpcPhaseV1(records.filter(r => r.kind === 'simulation_rpc' && r.ordinal === ordinal), control.supervision.phase_budgets.simulation, 'simulation');
  ...
  await simulateExactPreparedMessageV1({ message: Buffer.from(plan.message_base64, 'base64'), expected_message_sha256: plan.message_sha256,
    minimum_context_slot: floor, challenge: leg.challenge, assertFresh: async () => need(wall < leg.challenge.expires_at_unix_seconds),
