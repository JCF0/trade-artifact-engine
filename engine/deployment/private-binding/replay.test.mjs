...
for (const outcome of ['complete', 'acquisition-only', 'revoked', 'finalized-failure', 'unresolved']) {
  test(`fixed confined independent verifier consumes producer-emitted ${outcome} original bytes`, async () => {
    ...
    try {
      ...
      if (outcome === 'unresolved') { f.setVisible(false); f.setHandler(() => { throw Error('SYNTHETIC_UNRESOLVED'); }); }
      ...
      const result = replay(original, outcome);
      ...
      if (outcome === 'unresolved') { assert.equal(result.control.state.possible_submission, true); assert.equal(result.economic_observations.transactions.length, 0); }
      ...
      if (outcome === 'complete') {
        ...
        const selectedResult = replay(selected, 'complete-selected');
